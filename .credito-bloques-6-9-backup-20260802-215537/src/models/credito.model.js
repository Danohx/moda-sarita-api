    ...
    async (client) => {
      ...
      const { rows: paymentRows } = await client.query(
        `
          ...
          WHERE credito_id = $1::uuid
            AND estado = 'CONFIRMADO'
            AND concepto IN ('ENGANCHE_CREDITO', 'ABONO_CREDITO', 'LIQUIDACION_CREDITO')
          LIMIT 1
        `,
        [credito.id],
      );
      const { rows: applicationRows } = await client.query(
        `
          SELECT id
          FROM clientes.credito_aplicaciones_pago
          WHERE credito_id = $1::uuid
          LIMIT 1
        `,
        [credito.id],
      ...
      if (paymentRows.length || applicationRows.length) {
        throw modelError(
          "No se puede cancelar un crédito con abonos confirmados sin un proceso de reversión.",
        );
      }
      ...
      await client.query(
        `
          ...
          VALUES (
            $1, $2, $3, $4, now(), 'AJUSTE',
            'Cancelación de crédito', $5, $6, $7, NULL, $8
          )
        `,
        [
          credito.cliente_id,
