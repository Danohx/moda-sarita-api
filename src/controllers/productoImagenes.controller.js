...
export async function getProductoImagenes(req, res) {
  try {
    ...
    return res.status(500).json({ ok: false, msg: "Error listando imágenes", detail: err.message });
  }
}
...
export async function postProductoImagen(req, res) {
  try {
    ...
    if (!req.file) return res.status(400).json({ ok: false, msg: "Debes enviar un archivo (field: image)" });
    ...
    return res.status(500).json({ ok: false, msg: "Error subiendo imagen", detail: err.message });
  }
}
...
export async function patchProductoImagenPrincipal(req, res) {
  try {
    ...
    return res.status(500).json({ ok: false, msg: "Error marcando principal", detail: err.message });
  }
}
...
export async function deleteProductoImagenById(req, res) {
  try {
    ...
    const deleted = await deleteProductoImagenWithFallback(req.db, { productoId, imagenId });
    if (!deleted) 
    ...
    return res.status(500).json({ ok: false, msg: "Error eliminando imagen", detail: err.message });
  }
}
...
export async function patchProductoImagenesReorder(req, res) {
  try {
    ...
    if (!Array.isArray(items) || items.length === 0) {
      return res.status(400).json({ ok: false, msg: "items debe ser un arreglo no vacío" });
    }

    for (const it of items) {
      if (!it?.id) return res.status(400).json({ ok: false, msg: "Cada item debe tener id" });
      if (it.orden !== undefined && !Number.isInteger(it.orden)) {
        return res.status(400).json({ ok: false, msg: "orden debe ser entero" });
      }
      if (it.es_principal !== undefined && typeof it.es_principal !== "boolean") {
        return res.status(400).json({ ok: false, msg: "es_principal debe ser boolean" });
      }
    }
    ...
    return res.status(400).json({ ok: false, msg: "No se pudo reordenar", detail: err.message });
  }
}