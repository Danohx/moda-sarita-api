...
export const verifyLogin2FA = async (req, res) => {
  ...
  try {
    ...
    if (!valid)
      return res
        .status(401)
        .json({ success: false, mensaje: "Código 2FA inválido." });
...
export const sendMagicLink = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ mensaje: "Error al enviar el enlace mágico." });
  }
};
...
export const resetPassword = async (req, res) => {
  ...
  try {
    ...
    await req.db.query(
      "UPDATE seguridad.user_sessions SET revoked_at = now() WHERE user_id = $1",
      [userId],
    );
...
export const logout = async (req, res) => {
  ...
  try {
    const decoded = verifyRefreshToken(refreshToken);
    await req.db.query(
      "UPDATE seguridad.user_sessions SET revoked_at = now() WHERE id = $1",
      [decoded.sid],
    );
  } catch (_) {}
...
export const revokeAllSessions = async (req, res) => {
  ...
  try {
    ...
    return res.json({
      mensaje: "Se han cerrado todas las sesiones en todos los dispositivos.",
    });
  } catch (e) {
