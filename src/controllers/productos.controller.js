...
export async function getProductos(req, res) {
  try {
    ...
    return res.status(500).json({ ok: false, msg: "Error listando productos", detail: err.message });
  }
}
...
export async function getProductoById(req, res) {
  try {
    ...
    return res.status(500).json({ ok: false, msg: "Error obteniendo producto", detail: err.message });
  }
}

export async function postProducto(req, res) {
  try {
    if (!req.db) return res.status(500).json({ ok: false, msg: "DB context no configurado (req.db)" });
    ...
    if (!nombre || String(nombre).trim().length < 2) {
      return res.status(400).json({ ok: false, msg: "nombre requerido (mínimo 2 chars)" });
    }
    if (precio_costo === undefined || precio_venta === undefined) {
      return res.status(400).json({ ok: false, msg: "precio_costo y precio_venta son requeridos" });
    }
    ...
    if (err.code === "23505") {
      return res.status(409).json({ ok: false, msg: "SKU/slug/código de barras duplicado", detail: err.detail });
    }
    console.error("postProducto error:", err);
    return res.status(500).json({ ok: false, msg: "Error creando producto", detail: err.message });
  }
}
...
export async function patchProducto(req, res) {
  try {
    ...
    return res.status(500).json({ ok: false, msg: "Error actualizando producto", detail: err.message });
  }
}
...
export async function patchProductoDestacado(req, res) {
  try {
    ...
    if (typeof destacado !== "boolean") 
        return res.status(400).json({ ok: false, msg: "destacado debe ser boolean" });
    ...
    return res.status(500).json({ ok: false, msg: "Error cambiando destacado", detail: err.message });
  }
}
