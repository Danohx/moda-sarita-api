...
export async function patchVarianteStock(req, res) {
  try {
    ...
    if (!Number.isInteger(cantidad) || cantidad === 0) {
      return res.status(400).json({ ok: false, msg: "cantidad debe ser entero distinto de 0" });
    }
    if (!motivo || String(motivo).trim().length < 3) {
      return res.status(400).json({ ok: false, msg: "motivo es requerido (mínimo 3 caracteres)" });
    }
    ...
    return res.status(500).json({ ok: false, msg: "Error ajustando stock", detail: err.message });
  }
}
...
export async function getVariantesProducto(req, res) {
  try {
    ...
    return res.status(500).json({ ok: false, msg: "Error listando variantes", detail: err.message });
  }
}
...
export async function postVariantesProducto(req, res) {
  try {
    ...
    if (items.length === 0) return res.status(400).json({ ok: false, msg: "Body vacío" });
    ...
    for (const it of items) {
      ...
      const row = await createVariante(req.db, {
        ...
        codigo_barras: it?.codigo_barras ? String(it.codigo_barras).trim() : null,
        precio_venta: it?.precio_venta !== undefined && it?.precio_venta !== null ? Number(it.precio_venta) : null,
        precio_costo: it?.precio_costo !== undefined && it?.precio_costo !== null ? Number(it.precio_costo) : null,
    ...
    if (err.code === "23505") {
      return res.status(409).json({ ok: false, msg: "Variante duplicada (talla/color) o SKU/Barcode repetido", detail: err.detail });
    }
    console.error("postVariantesProducto error:", err);
    return res.status(500).json({ ok: false, msg: "Error creando variantes", detail: err.message });
  }
}
...
export async function patchVariante(req, res) {
  try {
    ...
    if (err.code === "23505") {
      return res.status(409).json({ ok: false, msg: "Variante duplicada o SKU/Barcode repetido", detail: err.detail });
    }
    console.error("patchVariante error:", err);
    return res.status(500).json({ ok: false, msg: "Error actualizando variante", detail: err.message });
  }
}
