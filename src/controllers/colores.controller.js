import { listColores, createColor, updateColor, setColorStatus } from "../models/colores.model.js";
...
export async function getColores(req, res) {
  try {
    ...
    return res.status(500).json({ ok: false, msg: "Error listando colores", detail: err.message });
  }
}
...
export async function postColor(req, res) {
  try {
    ...
    if (!isValidHex(hex)) {
      return res.status(400).json({ ok: false, msg: "hex inválido, usa formato #RRGGBB" });
    }

    const created = await createColor(req.db, { nombre: String(nombre).trim(), hex: hex ? String(hex).trim() : null });
    return res.status(201).json({ ok: true, data: created });
  } catch (err) {
    if (err.code === "23505") {
      return res.status(409).json({ ok: false, msg: "Color duplicado", detail: err.detail });
    }
    console.error("postColor error:", err);
    return res.status(500).json({ ok: false, msg: "Error creando color", detail: err.message });
  }
}
...
export async function patchColor(req, res) {
  try {
    ...
    return res.status(500).json({ ok: false, msg: "Error actualizando color", detail: err.message });
  }
}
